...
통일을 위해 정렬 알고리즘에서 사용할 기본 점수 규칙을 미리 정해 둔다.

일치 점수    2  
불일치 점수  -1  
갭 패널티    -2  

갭 점수 실험을 할 때는 이 값들 중에서 갭 패널티만 여러 값으로 바꾸어 사용한다.

## 역할 나누기

이 프로젝트는 세 사람이 나누어 할 것을 예상하고 있다. 함께 모이지 못하더라도 각자 맡은 부분만 채우면 전체 그림이 맞춰지도록 나눈다.

첫 번째 사람은 정렬 알고리즘을 직접 돌려 보는 역할을 맡는다. data 폴더의 example-pair.txt 에 사용할 단백질 서열 한 쌍을 고르고, src 폴더의 alignment 코드 파일 안에 정렬 알고리즘을 구현한 뒤, run-example 코드로 이 서열을 정렬해 본다. 이 사람이 만든 정렬 결과는 results 폴더의 example-alignment.txt 파일에 저장되며, 보고서에서 정렬 알고리즘 구현과 알고리즘별 정렬 결과 부분의 핵심 예시가 된다.

두 번째 사람은 실험과 BLAST 관련 작업을 맡는다. 첫 번째 사람이 정해 둔 서열 쌍과 점수 규칙을 그대로 사용하여 run-gap-penalty 코드로 갭 점수를 여러 값으로 바꿔 정렬을 반복하고, 각 설정에 대해 점수와 갭, 불일치 개수를 정리해서 gap-penalty-results.csv 파일에 정리한다. 또 같은 서열을 BLAST 검색에 사용해 정렬 부분을 blast-alignment.txt 파일에 붙여 두고, run-blast-compare 코드로 우리 정렬과 BLAST 정렬을 하나의 텍스트로 모아 results 폴더의 blast-comparison.txt 파일을 만든다.
...
처음 시작할 때는 다음 순서로 진행하면 된다.

첫째, data 폴더의 example-pair.txt 에 사용할 단백질 서열 한 쌍을 채운다.  
둘째, src 폴더의 alignment.py 파일 안에 smithWaterman 함수의 틀을 만든다.  
셋째, src 폴더의 run-example.py 파일에서 smithWaterman 을 한 번 호출해 예시 정렬이 나오는지 확인한다.  
이 세 단계가 준비되면 나머지 갭 점수 실험과 BLAST 비교 작업은 따로 진행할 수 있다.
